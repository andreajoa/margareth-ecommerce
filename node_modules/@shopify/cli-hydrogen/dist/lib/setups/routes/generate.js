...
import { getTemplateAppFile, ASSETS_STARTER_DIR_ROUTES, getStarterDir, getAssetsDir } from '../../../lib/build.js';
...
async function getResolvedRoutes(routeKeys = Object.keys(ROUTE_MAP)) {
  if (allRouteTemplateFiles.length === 0) {
    allRouteTemplateFiles = (await readdir(await getTemplateAppFile(ASSETS_STARTER_DIR_ROUTES))).map((item) => item.replace(/\.tsx?$/, ""));
  }
  const routeGroups = {};
