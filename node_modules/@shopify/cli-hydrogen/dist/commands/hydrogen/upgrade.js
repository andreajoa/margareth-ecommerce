...
async function displayDevUpgradeNotice({
  ...
  try {
    ...
    const isLatestHydrogenPackage = pinnedCurrentVersion === pinnedLatestVersion;
    const currentReleaseIndex = isLatestHydrogenPackage ? changelog.releases.findIndex(
    ...
    renderInfo({
      headline,
      body: [
        `Current: ${currentVersion} | Latest: ${pinnedLatestVersion}` + (isLatestHydrogenPackage ? " with updated dependencies" : "")
      ],
      customSections: [
          ...
          body: [
            {
              list: {
                items: [
                  `Run \`${cliCommand} upgrade\` or \`${cliCommand} upgrade --version ${relevantReleases[0]?.version ?? "<version>"}\``,
