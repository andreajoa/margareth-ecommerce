...
export async function loader({params, context}) {
  ...
  // Mapeia policies para pages ou usa policy da Shopify
  const redirectMap = {
    'politica-de-devolucao': '/pages/politica-de-devolucao',
    'politica-de-envio': '/pages/politica-de-envio',
    'politica-de-cookies': '/pages/politica-de-cookies',
    'refund-policy': '/pages/politica-de-devolucao',
    'shipping-policy': '/pages/politica-de-envio',
  };

  if (redirectMap[handle]) {
    return redirect(redirectMap[handle]);
  }

  // Para privacy-policy e terms-of-service usa a API da Shopify
  const policyMap = {
  ...
  const policyField = policyMap[handle];
    ...
    <div className="bg-[#FEFDF8] flex flex-col min-h-screen w-full">
      ...
      <main className="flex-grow w-full bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
          ...
          <div className="shopify-content" dangerouslySetInnerHTML={{__html: policy.body}} />
        </div>
      </main>
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          {footerMenu.map((group, idx) => (
            <div key={idx} className="text-center sm:text-left">
              <h3 className="font-bold text-[#0A3D2F] text-sm tracking-widest uppercase mb-4">
                {group.title}
              </h3>
